...
def editAddress():
    try:
        ...
        return jsonify({"edit-address": 1, "new-address": data["address"]})
    except Exception:
        return jsonify({"edit-address": -1})
...
def addAddress():
    try:
        ...
        db.session.commit()
        if not account:
            return jsonify({"add-address": -2, "message":"User record creation Failed"})
        account.address = data["address"]
        db.session.commit()

        return jsonify({"add-address": 1, "id":account.user_id, "address": data["address"]})
    except Exception as e:
        print(e)
        return jsonify({"add-address": -10})

@app.route('/address', methods=["DELETE"])
def deleteAddress():
    try:
        data = request.json
        account = Account.query.filter_by(user_id=data["user_id"])
        if not account:
            return jsonify({"status": -2, "message":"User Not Found"})
        db.session.delete(account)
        db.session.commit()

        return jsonify({"delete-address": 1})
    except Exception as e:
        return jsonify({"delete-address": -1, "message":str(e)})
